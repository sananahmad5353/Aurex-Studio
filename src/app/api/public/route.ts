...
export async function GET() {
  try {
    ...
    const [slides, services, testimonials, partners, reels, posts] = await Promise.all([
      ...
      db.instagramPost.findMany({ where: { active: true }, orderBy: { order: 'asc' } }),
    ]);

    return NextResponse.json({ settings: settingsMap, slides, services, testimonials, partners, reels, posts });
  } catch {
